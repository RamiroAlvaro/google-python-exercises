...
def find_even_index(arr):
    if sum(num for num in arr[1:]) == 0:
        return 0
    for i, n in enumerate(arr):
        if sum(num for num in arr[:i + 1]) == sum(num for num in arr[i + 2:]):
            return i + 1
    return -1
...
assert find_even_index([0,0,0,0,0]) == 0 #"Should pick the first index if more cases are valid"
assert find_even_index([-1,-2,-3,-4,-3,-2,-1]) == 3
